...
*For packages that have already been released, you can save time by running `curl -L https://k8s.kurl.sh/dist/kubernetes-1.25.4.tar.gz | tar -xzv -C kurl -f -` and `curl -L https://k8s.kurl.sh/dist/containerd-1.6.10.tar.gz | tar -xzv -C kurl -f -` on the test server.*  <br />
