...
function _dpkg_install_host_packages() {
    ...
    # Uninstall docker, if installed to install containerd only for new kURL installs
    ...
    if commandExists docker && [[ "${packages[*]}" == *"containerd.io"* ]] && [[ -n "${KURL_INSTALL_DIRECTORY_FLAG}" ]]; then
