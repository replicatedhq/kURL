...
function check_proxy_config() {
    if [ -n "$CONTAINERD_VERSION" ]; then
        ...
        local ip=$(cat "$config_file" | grep -oP '(?<=plugins\."io.containerd.grpc.v1.cri"\.registry\.configs\.").*(?="\.tls\])')
        local ca_file=$(cat "$config_file" | grep -oP '(?<=ca_file = ").*(?=")')
        ...
        local response=""
        if output=$(curl --cacert "$ca_file" "https://$ip/v2/" 2>&1); then
            response="$output"
        fi
        curl_exit_code=$?
