...
Add-on groups are directly related to _Installer Versions_, meaning that each _Installer Version_ will include a specific group of add-ons. This is a hypothetical example of an add-ons group for `installerVersion` version  `v2023.04.24-0` :
...
As we release new _Installer Versions_, the add-ons included in each one of them may be updated to reflect the latest features and functionalities. This, for example, could be the add-ons grouped under `installerVersion` version `v2023.04.25-0` (_versions for Containerd and Flannel were updated._):
...
_If no `installerVersion` parameter is specified in the Installer, the default behaviour is to install or update using the most up-to-dated Installer Version available_

In summary, users can select which add-ons and their respective versions they want by pinning a specific `installerVersion` in the Installer YAML.
