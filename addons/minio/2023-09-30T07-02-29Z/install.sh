...
function minio() {
    local src="$DIR/addons/minio/2023-09-30T07-02-29Z"
    ...
    if [ -n "$minio_ha_exists" ]; then
        ...
        kubectl set image -n minio statefulset/ha-minio minio=minio/minio:RELEASE.2023-09-30T07-02-29Z
        ...
        kubectl set image -n minio deployment/minio minio=minio/minio:RELEASE.2023-09-30T07-02-29Z 2>/dev/null || true
    else
...
function minio_create_fs_migration_deployment() {
    local src="$DIR/addons/minio/2023-09-30T07-02-29Z/migrate-fs"
