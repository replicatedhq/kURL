...
function minio() {
    local src="$DIR/addons/minio/2025-10-15T17-29-55Z"
    ...
    if [ -n "$minio_ha_exists" ]; then
        ...
        kubectl set image -n minio statefulset/ha-minio minio=proxy.replicated.com/library/minio:RELEASE.2025-10-15T17-29-55Z
        ...
        kubectl set image -n minio deployment/minio minio=proxy.replicated.com/library/minio:RELEASE.2025-10-15T17-29-55Z 2>/dev/null || true
    else
...
function minio_create_fs_migration_deployment() {
    local src="$DIR/addons/minio/2025-10-15T17-29-55Z/migrate-fs"
