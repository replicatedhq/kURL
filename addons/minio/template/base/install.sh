...
function minio() {
    ...
    if [ -n "$minio_ha_exists" ]; then
        ...
        kubectl set image -n minio statefulset/ha-minio minio=kotsadm/minio:__MINIO_VERSION__
        ...
        kubectl set image -n minio deployment/minio minio=kotsadm/minio:__MINIO_VERSION__ 2>/dev/null || true
    else
