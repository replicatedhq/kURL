...
function minio() {
    local src="$DIR/addons/minio/2025-02-28T09-55-16Z"
    ...
    if [ -n "$minio_ha_exists" ]; then
        ...
        kubectl set image -n minio statefulset/ha-minio minio=minio/minio:RELEASE.2025-02-28T09-55-16Z
        ...
        kubectl set image -n minio deployment/minio minio=minio/minio:RELEASE.2025-02-28T09-55-16Z 2>/dev/null || true
    else
...
function minio_create_fs_migration_deployment() {
    local src="$DIR/addons/minio/2025-02-28T09-55-16Z/migrate-fs"
