...
function kotsadm() {
    local src="$DIR/addons/kotsadm/1.22.0"
    ...
    if [ -n "$PROXY_ADDRESS" ]; then
        ...
        render_yaml_file "$DIR/addons/kotsadm/1.22.0/tmpl-kotsadm-proxy.yaml" > "$DIR/kustomize/kotsadm/kotsadm-proxy.yaml"
        insert_patches_strategic_merge "$DIR/kustomize/kotsadm/kustomization.yaml" kotsadm-proxy.yaml
    fi

    if [ "$AIRGAP" == "1" ]; then
        cp "$DIR/addons/kotsadm/1.22.0/kotsadm-airgap.yaml" "$DIR/kustomize/kotsadm/kotsadm-airgap.yaml"
...
function kotsadm_join() {
    kotsadm_cli "$DIR/addons/kotsadm/1.22.0"
}
...
function kotsadm_secret_cluster_token() {
    ...
    render_yaml_file "$DIR/addons/kotsadm/1.22.0/tmpl-secret-cluster-token.yaml" > "$DIR/kustomize/kotsadm/secret-cluster-token.yaml"
...
function kotsadm_secret_authstring() {
    ...
    render_yaml_file "$DIR/addons/kotsadm/1.22.0/tmpl-secret-authstring.yaml" > "$DIR/kustomize/kotsadm/secret-authstring.yaml"
...
function kotsadm_secret_password() {
    ...
    render_yaml_file "$DIR/addons/kotsadm/1.22.0/tmpl-secret-password.yaml" > "$DIR/kustomize/kotsadm/secret-password.yaml"
...
function kotsadm_secret_postgres() {
    ...
    render_yaml_file "$DIR/addons/kotsadm/1.22.0/tmpl-secret-postgres.yaml" > "$DIR/kustomize/kotsadm/secret-postgres.yaml"
...
function kotsadm_secret_s3() {
    ...
    render_yaml_file "$DIR/addons/kotsadm/1.22.0/tmpl-secret-s3.yaml" > "$DIR/kustomize/kotsadm/secret-s3.yaml"
...
function kotsadm_secret_session() {
    ...
    render_yaml_file "$DIR/addons/kotsadm/1.22.0/tmpl-secret-session.yaml" > "$DIR/kustomize/kotsadm/secret-session.yaml"
...
function kotsadm_api_encryption_key() {
    ...
    render_yaml_file "$DIR/addons/kotsadm/1.22.0/tmpl-secret-api-encryption.yaml" > "$DIR/kustomize/kotsadm/secret-api-encryption.yaml"
...
function kotsadm_api_patch_prometheus() {
    ...
    cp "$DIR/addons/kotsadm/1.22.0/patches/api-prometheus.yaml" "$DIR/kustomize/kotsadm/api-prometheus.yaml"
}
...
function kotsadm_cli() {
    ...
    if [ ! -f "$src/assets/kots.tar.gz" ] && [ "$AIRGAP" != "1" ]; then
        mkdir -p "$src/assets"
        curl -L "https://github.com/replicatedhq/kots/releases/download/v1.22.0/kots_linux_amd64.tar.gz" > "$src/assets/kots.tar.gz"
    fi
...
function kotsadm_cacerts_file() {
    ...
    if [ -n "$KOTSADM_TRUSTED_CERT_MOUNT" ]; then
        render_yaml_file "$DIR/addons/kotsadm/1.22.0/tmpl-kotsadm-cacerts.yaml" > "$DIR/kustomize/kotsadm/kotsadm-cacerts.yaml"
