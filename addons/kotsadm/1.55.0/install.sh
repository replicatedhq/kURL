...
function kotsadm() {
    local src="$DIR/addons/kotsadm/1.55.0"
    ...
    if [ "$KOTSADM_DISABLE_S3" == "1" ]; then
        cp "$DIR/addons/kotsadm/1.55.0/statefulset/kotsadm-statefulset.yaml" "$DIR/kustomize/kotsadm/kotsadm-statefulset.yaml"
        ...
        cp "$DIR/addons/kotsadm/1.55.0/deployment/kotsadm-deployment.yaml" "$DIR/kustomize/kotsadm/kotsadm-deployment.yaml"
    ...
    if [ -n "$PROXY_ADDRESS" ]; then
        ...
        if [ "$KOTSADM_DISABLE_S3" == "1" ]; then
            render_yaml_file "$DIR/addons/kotsadm/1.55.0/statefulset/tmpl-kotsadm-proxy.yaml" > "$DIR/kustomize/kotsadm/kotsadm-proxy.yaml"
        else
            render_yaml_file "$DIR/addons/kotsadm/1.55.0/deployment/tmpl-kotsadm-proxy.yaml" > "$DIR/kustomize/kotsadm/kotsadm-proxy.yaml"
        fi
    ...
    if [ "$AIRGAP" == "1" ]; then
        if [ "$KOTSADM_DISABLE_S3" == "1" ]; then
            cp "$DIR/addons/kotsadm/1.55.0/statefulset/kotsadm-airgap.yaml" "$DIR/kustomize/kotsadm/kotsadm-airgap.yaml"
        else
            cp "$DIR/addons/kotsadm/1.55.0/deployment/kotsadm-airgap.yaml" "$DIR/kustomize/kotsadm/kotsadm-airgap.yaml"
        fi
    ...
    if [ -n "$INSTALLATION_ID" ]; then
        if [ "$KOTSADM_DISABLE_S3" == "1" ]; then
            render_yaml_file "$DIR/addons/kotsadm/1.55.0/statefulset/tmpl-kotsadm-installation-id.yaml" > "$DIR/kustomize/kotsadm/kotsadm-installation-id.yaml"
        else
            render_yaml_file "$DIR/addons/kotsadm/1.55.0/deployment/tmpl-kotsadm-installation-id.yaml" > "$DIR/kustomize/kotsadm/kotsadm-installation-id.yaml"
        fi
...
function kotsadm_join() {
    kotsadm_cli "$DIR/addons/kotsadm/1.55.0"
}
...
function kotsadm_secret_cluster_token() {
    ...
    render_yaml_file "$DIR/addons/kotsadm/1.55.0/tmpl-secret-cluster-token.yaml" > "$DIR/kustomize/kotsadm/secret-cluster-token.yaml"
...
function kotsadm_secret_authstring() {
    ...
    render_yaml_file "$DIR/addons/kotsadm/1.55.0/tmpl-secret-authstring.yaml" > "$DIR/kustomize/kotsadm/secret-authstring.yaml"
...
function kotsadm_secret_password() {
    ...
    render_yaml_file "$DIR/addons/kotsadm/1.55.0/tmpl-secret-password.yaml" > "$DIR/kustomize/kotsadm/secret-password.yaml"
...
function kotsadm_secret_postgres() {
    ...
    render_yaml_file "$DIR/addons/kotsadm/1.55.0/tmpl-secret-postgres.yaml" > "$DIR/kustomize/kotsadm/secret-postgres.yaml"
...
function kotsadm_secret_dex_postgres() {
    ...
    render_yaml_file "$DIR/addons/kotsadm/1.55.0/tmpl-secret-dex-postgres.yaml" > "$DIR/kustomize/kotsadm/secret-dex-postgres.yaml"
...
function kotsadm_secret_s3() {
    ...
    render_yaml_file "$DIR/addons/kotsadm/1.55.0/tmpl-secret-s3.yaml" > "$DIR/kustomize/kotsadm/secret-s3.yaml"
...
function kotsadm_secret_session() {
    ...
    render_yaml_file "$DIR/addons/kotsadm/1.55.0/tmpl-secret-session.yaml" > "$DIR/kustomize/kotsadm/secret-session.yaml"
...
function kotsadm_api_encryption_key() {
    ...
    render_yaml_file "$DIR/addons/kotsadm/1.55.0/tmpl-secret-api-encryption.yaml" > "$DIR/kustomize/kotsadm/secret-api-encryption.yaml"
...
function kotsadm_api_patch_s3_migration() {
    ...
    cp "$DIR/addons/kotsadm/1.55.0/statefulset/patches/s3-migration.yaml" "$DIR/kustomize/kotsadm/s3-migration.yaml"
}

function kotsadm_api_patch_prometheus() {
    if [ "$KOTSADM_DISABLE_S3" == "1" ]; then
        cp "$DIR/addons/kotsadm/1.55.0/statefulset/patches/api-prometheus.yaml" "$DIR/kustomize/kotsadm/api-prometheus.yaml"
    else
        cp "$DIR/addons/kotsadm/1.55.0/deployment/patches/api-prometheus.yaml" "$DIR/kustomize/kotsadm/api-prometheus.yaml"
    fi
...
function kotsadm_cli() {
    ...
    if [ ! -f "$src/assets/kots.tar.gz" ] && [ "$AIRGAP" != "1" ]; then
        mkdir -p "$src/assets"
        curl -L "https://github.com/replicatedhq/kots/releases/download/v1.55.0/kots_linux_amd64.tar.gz" > "$src/assets/kots.tar.gz"
    fi
...
function kotsadm_cacerts_file() {
    ...
    if [ -n "$KOTSADM_TRUSTED_CERT_MOUNT" ]; then
        if [ "$KOTSADM_DISABLE_S3" == "1" ]; then
            render_yaml_file "$DIR/addons/kotsadm/1.55.0/statefulset/tmpl-kotsadm-cacerts.yaml" > "$DIR/kustomize/kotsadm/kotsadm-cacerts.yaml"
        else
            render_yaml_file "$DIR/addons/kotsadm/1.55.0/deployment/tmpl-kotsadm-cacerts.yaml" > "$DIR/kustomize/kotsadm/kotsadm-cacerts.yaml"
        fi
...
function kotsadm_preflight() {
    echo "${DIR}/addons/kotsadm/1.55.0/host-preflight.yaml"
}
