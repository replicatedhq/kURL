...
function velero_install() {
    ...
    if ! kubernetes_resource_exists "$VELERO_NAMESPACE" backupstoragelocation default; then
        ...
        if [ "$KOTSADM_DISABLE_S3" == 1 ] && velero_rwx_storage_class_exists ; then
    ...
    "$src"/assets/velero-v"${VELERO_VERSION}"-linux-amd64/velero install \
        ...
        --plugins velero/velero-plugin-for-aws:v1.12.2,velero/velero-plugin-for-gcp:v1.12.2,velero/velero-plugin-for-microsoft-azure:v1.12.2,replicated/local-volume-provider:0.6.11,"$KURL_UTIL_IMAGE" \
