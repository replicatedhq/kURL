...
function velero_install() {
    ...
    "$src"/assets/velero-v"${VELERO_VERSION}"-linux-amd64/velero install \
        ...
        --plugins velero/velero-plugin-for-aws:v__AWS_PLUGIN_VERSION__,velero/velero-plugin-for-gcp:v__GCP_PLUGIN_VERSION__,velero/velero-plugin-for-microsoft-azure:v__AZURE_PLUGIN_VERSION__,replicated/local-volume-provider:__LOCAL_VOLUME_PROVIDER_VERSION__,"$KURL_UTIL_IMAGE" \
