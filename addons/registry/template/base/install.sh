...
function registry_install() {
    ...
    if ! registry_pvc_exists && object_store_exists; then
        ...
        cp "$DIR/addons/registry/__registry_version__/patch-deployment-velero.yaml" "$DIR/kustomize/registry/patch-deployment-velero.yaml"
...
function regsitry_init_service() {
    ...
    if [ -n "$REGISTRY_PUBLISH_PORT" ]; then
        render_yaml_file "$DIR/addons/registry/__registry_version__/tmpl-node-port-patch.yaml" > "$DIR/kustomize/registry/node-port-patch.yaml"
        insert_patches_strategic_merge "$DIR/kustomize/registry/kustomization.yaml" "node-port-patch.yaml"
    fi

    if [ -n "$KURL_REGISTRY_IP" ]; then
        render_yaml_file "$DIR/addons/registry/__registry_version__/tmpl-cluster-ip-patch.yaml" > "$DIR/kustomize/registry/cluster-ip-patch.yaml"
...
function registry_session_secret() {
    ...
    render_yaml_file "$DIR/addons/registry/__registry_version__/tmpl-secret.yaml" > "$DIR/kustomize/registry/secret.yaml"
}
...
function registry_pki_secret() {
    ...
    local tmp="$DIR/addons/registry/__registry_version__/tmp"
    rm -rf "$tmp"
    mkdir -p "$tmp"
    pushd "$tmp"
