...
function registry_install() {
    ...
    if ! registry_pvc_exists && object_store_exists; then
        ...
        render_yaml_file "$DIR/addons/registry/3.0.0/tmpl-deployment-objectstore.yaml" > "$DIR/kustomize/registry/deployment-objectstore.yaml"
        insert_resources "$DIR/kustomize/registry/kustomization.yaml" deployment-objectstore.yaml

        cp "$DIR/addons/registry/3.0.0/patch-deployment-velero.yaml" "$DIR/kustomize/registry/patch-deployment-velero.yaml"
        insert_patches_strategic_merge "$DIR/kustomize/registry/kustomization.yaml" patch-deployment-velero.yaml
        render_yaml_file "$DIR/addons/registry/3.0.0/tmpl-configmap-velero.yaml" > "$DIR/kustomize/registry/configmap-velero.yaml"
        ...
        cp "$DIR/addons/registry/3.0.0/deployment-pvc.yaml" "$DIR/kustomize/registry/deployment-pvc.yaml"
        render_yaml_file "$DIR/addons/registry/3.0.0/tmpl-persistentvolumeclaim.yaml" > "$DIR/kustomize/registry/persistentvolumeclaim.yaml"
    ...
    if registry_will_migrate_pvc; then
        ...
        render_yaml_file "$DIR/addons/registry/3.0.0/tmpl-configmap-migrate-s3.yaml" > "$DIR/kustomize/registry/configmap-migrate-s3.yaml"
        insert_resources "$DIR/kustomize/registry/kustomization.yaml" configmap-migrate-s3.yaml
        cp "$DIR/addons/registry/3.0.0/patch-deployment-migrate-s3.yaml" "$DIR/kustomize/registry/patch-deployment-migrate-s3.yaml"
        insert_patches_strategic_merge "$DIR/kustomize/registry/kustomization.yaml" patch-deployment-migrate-s3.yaml
    fi

    render_yaml_file "$DIR/addons/registry/3.0.0/tmpl-troubleshoot.yaml" > "$DIR/kustomize/registry/troubleshoot.yaml"
...
function regsitry_init_service() {
    ...
    cp "$DIR/addons/registry/3.0.0/kustomization.yaml" "$DIR/kustomize/registry/kustomization.yaml"

    cp "$DIR/addons/registry/3.0.0/service.yaml" "$DIR/kustomize/registry/service.yaml"
    ...
    if [ -n "$REGISTRY_PUBLISH_PORT" ]; then
        render_yaml_file "$DIR/addons/registry/3.0.0/tmpl-node-port-patch.yaml" > "$DIR/kustomize/registry/node-port-patch.yaml"
        insert_patches_strategic_merge "$DIR/kustomize/registry/kustomization.yaml" "node-port-patch.yaml"
    fi

    if [ -n "$KURL_REGISTRY_IP" ]; then
        render_yaml_file "$DIR/addons/registry/3.0.0/tmpl-cluster-ip-patch.yaml" > "$DIR/kustomize/registry/cluster-ip-patch.yaml"
...
function registry_session_secret() {
    ...
    render_yaml_file "$DIR/addons/registry/3.0.0/tmpl-secret.yaml" > "$DIR/kustomize/registry/secret.yaml"
}
...
function registry_pki_secret() {
    ...
    local tmp="$DIR/addons/registry/3.0.0/tmp"
    rm -rf "$tmp"
    mkdir -p "$tmp"
    pushd "$tmp"
