...
function registry_install() {
    ...
    if ! registry_pvc_exists && object_store_exists; then
        ...
        render_yaml_file "$DIR/addons/registry/2.8.3/tmpl-deployment-objectstore.yaml" > "$DIR/kustomize/registry/deployment-objectstore.yaml"
        insert_resources "$DIR/kustomize/registry/kustomization.yaml" deployment-objectstore.yaml

        cp "$DIR/addons/registry/2.8.3/patch-deployment-velero.yaml" "$DIR/kustomize/registry/patch-deployment-velero.yaml"
        insert_patches_strategic_merge "$DIR/kustomize/registry/kustomization.yaml" patch-deployment-velero.yaml
        render_yaml_file "$DIR/addons/registry/2.8.3/tmpl-configmap-velero.yaml" > "$DIR/kustomize/registry/configmap-velero.yaml"
        ...
        cp "$DIR/addons/registry/2.8.3/deployment-pvc.yaml" "$DIR/kustomize/registry/deployment-pvc.yaml"
        render_yaml_file "$DIR/addons/registry/2.8.3/tmpl-persistentvolumeclaim.yaml" > "$DIR/kustomize/registry/persistentvolumeclaim.yaml"
...
function regsitry_init_service() {
    ...
    if [ -n "$REGISTRY_PUBLISH_PORT" ]; then
        render_yaml_file "$DIR/addons/registry/2.8.3/tmpl-node-port-patch.yaml" > "$DIR/kustomize/registry/node-port-patch.yaml"
        insert_patches_strategic_merge "$DIR/kustomize/registry/kustomization.yaml" "node-port-patch.yaml"
    fi

    if [ -n "$KURL_REGISTRY_IP" ]; then
        render_yaml_file "$DIR/addons/registry/2.8.3/tmpl-cluster-ip-patch.yaml" > "$DIR/kustomize/registry/cluster-ip-patch.yaml"
...
function registry_session_secret() {
    ...
    render_yaml_file "$DIR/addons/registry/2.8.3/tmpl-secret.yaml" > "$DIR/kustomize/registry/secret.yaml"
}
...
function registry_pki_secret() {
    ...
    local tmp="$DIR/addons/registry/2.8.3/tmp"
    rm -rf "$tmp"
    mkdir -p "$tmp"
    pushd "$tmp"
