...
function weave() {
    cp "$DIR/addons/weave/2.7.0/kustomization.yaml" "$DIR/kustomize/weave/kustomization.yaml"
    cp "$DIR/addons/weave/2.7.0/rbac.yaml" "$DIR/kustomize/weave/rbac.yaml"
    cp "$DIR/addons/weave/2.7.0/daemonset.yaml" "$DIR/kustomize/weave/daemonset.yaml"
...
function weave_resource_secret() {
    ...
    render_yaml_file "$DIR/addons/weave/2.7.0/tmpl-secret.yaml" > "$DIR/kustomize/weave/secret.yaml"
}

function weave_patch_encrypt() {
    insert_patches_strategic_merge "$DIR/kustomize/weave/kustomization.yaml" encrypt.yaml
    cp "$DIR/addons/weave/2.7.0/encrypt.yaml" "$DIR/kustomize/weave/encrypt.yaml"
}
...
function weave_patch_ip_alloc_range() {
    ...
    render_yaml_file "$DIR/addons/weave/2.7.0/tmpl-ip-alloc-range.yaml" > "$DIR/kustomize/weave/ip-alloc-range.yaml"
}
...
function weave_preflight() {
    echo "${DIR}/addons/weave/2.7.0/host-preflight.yaml"
}
